...
namespace data_structures_assignment6
{
    ...
    // to a copy of the removed item.
    {
        ...
        /* BASE CASE: When there is no right child of the root_ptr node */
        {
            removed = root_ptr->data();
            binary_tree_node <Item>* temp = root_ptr;   // The root_ptr should be moved down
            root_ptr = root_ptr->left();                // to its left child and then the original
            delete temp;                                // root node must be deleted
        }
        else /* If root does have aright child */
        {
            bst_remove_max(root_ptr -> right(), removed);
            // In this case, a recursive call can be made
    ...
    // Postcondition: A new copy of entry has been inserted into the bag
    {
        ...
        else
        {
            ...
            while (check == 0)
            {
                ...
                {
                    if ( cursor->left() == NULL ){
                        // Create a node and inserts it to the left
                ...
                else
                {
                    if ( cursor->right() == NULL ){
                        // Create a node and inserts it to the right
                        binary_tree_node<Item> *temp;
                        temp = new binary_tree_node<Item>(entry);
                        cursor->right()=temp;
                        check = 1;
    ...
    // Return value is number of times target is in the bag
    {
        size_type answer = 0;
        binary_tree_node<Item> *cursor;
